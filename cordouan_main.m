...
func_smp = build_transfer_function(input.sample);
func_ref = build_transfer_function(input.reference);
...
%p1 = @(f, n) exp(-1i*f*2*pi*1e12*n*115/(3e8*1e6));
%f_ana = @(f,n) (8/9)*(p1(f,n) + (1/9)*p1(f,n)^3);
