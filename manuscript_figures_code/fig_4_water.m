...
path = fullfile(nelly_path, 'manuscript_figures_code',...
    'fig_4_data', 'water_in_cuvette_results', filesep);
...
eps_sim_re = importdata([path 'water_eps_re.tsv']);
eps_sim_re = eps_sim_re.data;
eps_sim_im = importdata([path 'water_eps_im.tsv']);
...
[freq, n_fit_full, freq_full, tf_full, tf_spec, tf_pred, func, spec_smp, spec_ref]...
    = nelly_main([path 'water_dispersion_air_referenced_input.json'],...
...
[~, n_simple] = just_propagation(freq, tf_spec, 100);
